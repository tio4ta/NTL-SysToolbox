...
Vérifier pour chaque OS s’il est encore supporté, en fin de vie imminente ou déjà hors support, via l’API endoflife.date.

Générer automatiquement des rapports exploitables (CSV, JSON, HTML) pour documenter l’état d’obsolescence du parc.
...
Scan réseau d’une plage IP avec Nmap et export en XML.

Parsing du fichier XML pour reconstruire une liste d’hôtes avec : IP, hostname/CN, OS, version.

Normalisation de l’OS et de la version (Windows, Windows Server, Ubuntu, Debian, Linux).

Appel à l’API endoflife.date pour chaque couple OS/version afin de récupérer les dates de support et EOL.

Calcul du statut : SUPPORTÉ, EOL < 1 AN, EOL IMMINENT, NON SUPPORTÉ, ou cas particuliers (version inconnue, non détectable).

Affichage console + génération de rapports CSV/JSON/HTML dans un dossier de sortie.
